...
import Phase1AttackPanel from '@/app/components/Phase1AttackPanel';
import { SESSION_KEY_STUB, phase1Report } from '@/lib/phase1-data';
import {
  Chart as ChartJS,
  CategoryScale,
  LinearScale,
...
export default function QuantumGuardDashboard() {
  ...
  useEffect(() => {
    ...
    setAttackStep(1);
    const timers = [2, 3, 4, 5].map((step, i) =>
      window.setTimeout(() => setAttackStep(step), (i + 1) * 700)
    );
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const rsaUnwrap = isAttackSimulated && pqcMode === 'RSA-2048';
      const hex = Array.from({ length: 24 }, () => Math.floor(Math.random() * 16).toString(16)).join('');

      let payload: string;
      if (rsaUnwrap) {
        payload = `HARVESTED wrap=${phase1Report.intercepted.ciphertext} → AES_STUB=${SESSION_KEY_STUB} (d=${phase1Report.recovered_d})`;
      } else if (isAttackSimulated && pqcMode === 'HYBRID') {
        payload = `HYBRID: RSA limb factored; ML-KEM_CT[${hex.slice(0, 16)}] still sealed`;
      } else if (isSecured) {
        payload = `PQC_CT[ML-KEM]: ${hex}`;
      } else {
        payload = `RSA_WRAP c=${phase1Report.intercepted.ciphertext}  (harvestable)`;
      }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-6">
      ...
      <Phase1AttackPanel attackActive={isAttackSimulated} pqcMode={pqcMode} activeStep={attackStep} />
    </div>
  );
}